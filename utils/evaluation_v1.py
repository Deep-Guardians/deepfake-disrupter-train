...
def validate(dataloader, perturbation_generator, deepfake_detector, fake_image_generator, face_detector, criterion_bce, device, sample_img):
    perturbation_generator.eval()
    all_labels = []
    all_preds = []
    all_detection_loss_real = 0
        ...
        for real_images in dataloader:
            ...
            for idx, img in enumerate(perturbed_images):
                ...
                try:
                    ...
                    fake_image_cv = fake_image_generator.get(cv_img, train_face[0], sample_img, paste_back=True)
            ...
            all_detection_loss_real+= detection_loss_real.item()
            all_detection_loss_fake+=detection_loss_fake.item()
            all_identity_loss+=identity_loss.item()

            # Collect predictions and labels
            real_preds = (outputs_real > 0.5).int().view(-1).cpu().numpy()
            fake_preds = (outputs_fake < 0.5).int().view(-1).cpu().numpy()
    ...
    all_detection_loss_real /= counts
    ...
    wandb.log({
        ...
        'val_detection_loss_real': all_detection_loss_real,
        'val_detection_loss_fake': all_detection_loss_fake,
